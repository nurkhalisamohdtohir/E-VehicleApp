...
<?php
    ...
    if(isset($_GET['Service_No'])) {
	    ...
	    $query ="SELECT *, DATE_FORMAT(Service_Date, '%d/%m/%Y') AS Sv_Date, DATE_FORMAT(Next_Date, '%d/%m/%Y') AS Nx_Date FROM `vehicle` v , `vehiclecat` c,`service` s  WHERE v.`Cat_No` = c.`Cat_No` AND v.`Vehicle_Plate` = s.`Vehicle_Plate` AND Service_No='$Service_No'";
...
if(isset($_POST['Update'])) {
	...
	$sql = "UPDATE service SET Service_Date='$Service_Date', Service_Mileage='$Service_Mileage', Next_Date='$Next_Date', Next_Mileage='$Next_Mileage' WHERE Service_No='$Service_No'";
...
?>
		...
		<script type="text/javascript">
        	alert('Service record is updated!');
...
<content>
	<center>
        ...
        <div class="title">
			<h2><b>Update Service Form</b></h2><hr>
		</div>

		<div class="form-box">
			<!-- <div class="form-left"> -->
				<form role="form"  id="serviceForm"  action="service_update.php" method="POST">
					...
					<div class="form">
						<div class="tarikh-pergi">
							<label><b>Service Date : </b></label><br>
				  			<input type="date" name="Service_Date" value="<?php echo "$row[Service_Date]"?>" required>
						</div>

						<div class="tarikh-balik">
							<label><b>Service Mileage : </b></label><br>
				  			<input type="text" id="currentMileage" name="Service_Mileage" value="<?php echo "$row[Service_Mileage]"?>" required onkeyup="checkMileage()">
						</div>
					</div>
					...
					<div class="form">
						<div class="masa-pergi">
							<label><b>Next Date : </b></label><br>
				  			<input type="date" name="Next_Date" value="<?php echo "$row[Next_Date]"?>" required><br>
						</div>

						<div class="masa-balik">
							<label><b>Next Mileage : </b></label><br>
				  			<input type="text"  id="serviceMileage" name="Next_Mileage" value="<?php echo "$row[Next_Mileage]"?>" required><br>
						</div>
					</div>
					...
					<div class="form">
						<input type="hidden" name="Service_No" value=<?php echo $row['Service_No'];?>>
					</div>
			...
			<div class="form-right">
				 -->
					...
					<div class="form">
						<input type="checkbox" name="catatan" value="" required checked><strong> I acknowledge that all information are correct.</strong>
					</div>
    ...
    <div class="title">
		...
		<div class="div-table">
            <table class= "table-semak">
				...
				$counter = 1;
				$sql="SELECT *, DATE_FORMAT(Service_Date, '%d/%m/%Y') AS S_Date, DATE_FORMAT(Next_Date, '%d/%m/%Y') AS N_Date FROM `vehicle` v , `vehiclecat` c, `service` s WHERE v.`Cat_No` = c.`Cat_No` AND v.`Vehicle_Plate` = s.`Vehicle_Plate`";
                ...
                <tr class="tbody" id="MyTable">
                    ...
                    <td><?php echo $row1['S_Date'] ?></td>

                    <td><?php echo $row1['Service_Mileage'] ?></td>

                    <td><?php echo $row1['N_Date'] ?></td>
